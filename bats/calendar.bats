...
@test "dry-run create emits a valid preview envelope with parsed alarm + recurrence" {
  run $BIN calendar events create \
...
@test "dry-run delete emits a delete preview" {
  run $BIN calendar events delete --id SOME-ID --span all
  [ "$status" -eq 0 ]
...
@test "all-day is NOT inferred from bare dates (MCP parity)" {
  run $BIN calendar events create --title "apple-cli-test x" --start 2026-07-15 --end 2026-07-16
  [ "$status" -eq 0 ]
  echo "$output" | grep -q '"is_all_day" : false'
}

@test "title-only structured location is accepted (MCP parity)" {
  run $BIN calendar events create --title "apple-cli-test y" \
...
@test "end before start is rejected (validation_error)" {
  run $BIN calendar events create --title "apple-cli-test w" \
...
@test "contradictory --clear-alarms + --alarm is rejected" {
  run $BIN calendar events update --id X --clear-alarms --alarm 15m
  [ "$status" -eq 64 ]
  echo "$output" | grep -q '"type" : "validation_error"'
}
...
@test "bad recurrence weekday yields validation_error / exit 64" {
  run $BIN calendar events create --title "apple-cli-test x" --start 2026-07-20 --end 2026-07-20 --recurrence "freq=weekly;byday=8"
  [ "$status" -eq 64 ]
  echo "$output" | grep -q '"type" : "validation_error"'
...
@test "bad date yields validation_error / exit 64" {
  run $BIN calendar events create --title "apple-cli-test x" --start "2026-13-45" --end "2026-07-20"
  [ "$status" -eq 64 ]
  echo "$output" | grep -q '"type" : "validation_error"'
}

@test "bad availability yields validation_error / exit 64" {
  run $BIN calendar events create --title "apple-cli-test x" --start 2026-07-20 --end 2026-07-20 --availability wat
  [ "$status" -eq 64 ]
  echo "$output" | grep -q '"type" : "validation_error"'
}

@test "missing required --title exits 64 (usage)" {
  run $BIN calendar events create --start 2026-07-20 --end 2026-07-20
  [ "$status" -eq 64 ]
}
