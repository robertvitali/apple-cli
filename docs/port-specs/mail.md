...
| 11 | Send — HTML body | B `compose_email(body_html)` | CORE | body_html + plain fallback | **B-only.** The generated `.eml` `--out` path refuses a Foundation-selected final-leaf symlink before Mail access and rechecks immediately before execute writes. Refusal is `safety_violation` / 77 and beats generic file-write failures. The check inspects the spellings Foundation actually selects for the write (the constructed file-URL path when it resolves physically, and the symlink-resolved path always), so a link hidden behind an absent `..` component is refused too. The write itself is atomic, so a link planted after the final check is replaced rather than followed; a residual time-of-check race remains between that check and the write. |
...
| 25 | Save attachments | A `save_attachments(message_id, dir, indices)`, B `save_email_attachment(subject_keyword, name, path)` | CORE | A: 0-based indices or all → dir · B: single by name → path | **Conflict:** A index/all; B by-name. Union = both. Destination confinement, Foundation-selected `--out` final-leaf symlink refusal, and filesystem-shape validation run before Envelope Index or Mail.app access. The guard and confinement use the same original `--out` spelling: removing terminal `/.` or `/./` can select an unrelated leaf through a symlinked parent and `..`. The captured save destination is unchanged; both existing and dangling selected links are refused on preview and execution with `safety_violation` / 77. Immediately before an execute save, both modes verify that the destination parent still resolves to the preflight path and remains a directory, then refuse newly planted leaf symlinks; `--dir` also refuses a regular file that appears after its initial no-clobber check. This narrows symlink reparenting to the interval between the final host check and Mail.app's separate-process save. A same-path rename swap between two real directories is also outside the path-based guard. The existing conservative parent check is unchanged: a terminal-dot path whose captured parent retains a symlink alias may preview successfully but refuse execution even when that alias is stable. DIVERGENCE (missing/non-directory `--out` parent): the CLI returns typed `validation_error` / 64 before store access. Oracle B creates no parent and saves no file, but its bare AppleScript `try` suppresses the save failure and misleadingly reports the attachment as not found. No oracle write capability is dropped. |
