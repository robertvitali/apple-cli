...
| Sub-op | Class | Key params | Behavior |
|---|---|---|---|
| read | CORE | `id?`, `filterCalendar`, `filterAccount`, `search`, `availability`, `startDate`, `endDate` | ±14d default window. Returns calendars+events (event JSON incl. organizer/attendees/status — READ-only). |
