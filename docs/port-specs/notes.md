...
| 13 | search-notes | C | `query`, `searchContent`(bool), `account`, `folder`, `modifiedSince`, `limit` | Title (or body) search; returns `{title,folder,id}[]`. Flags `Recently Deleted`. Sync-aware. |
