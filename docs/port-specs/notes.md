...
| 2e | update-note (response title) | C | `format`, `newTitle`, `newContent` | In **html** format the reported title is DERIVED from the new body via `firstVisibleHtmlLine` and `newTitle` is IGNORED — Notes takes a note's title from its first rendered line, so reporting `newTitle` would name something Notes never shows. Falls back to the current title when the body renders nothing. **plaintext** keeps JS truthiness: an empty `newTitle` falls back rather than blanking. Uses a separate oracle-faithful entity decoder (optional semicolon, two sequential whole-string passes hex-then-decimal, surrogate/range rejection). The split mirrors the ORACLE, which ships MULTIPLE decode paths — this one, the regex plaintext converter `htmlToPlaintext` mirrors verbatim, and (the one `get-note-markdown` actually shadows) turndown over a REAL DOM (index.js:41694 at 2.7.5). UPDATE 2026-08-19 (NOTES-L1, live-measured): the markdown-path `decodeEntities` now decodes the HTML5 LEGACY names (amp/lt/gt/quot/nbsp) with optional semicolons — Notes.app serialized a literal unterminated `&amp ` in a real note and the oracle's DOM decoded it where the CLI left `&amp` (user text corrupted on read; fixed + revert-red pinned) — and `&nbsp;` now decodes to U+00A0 on that path (turndown's collapseWhitespace folds only `[ \r\n\t]`, so U+00A0 survives into oracle markdown; `htmlToPlaintext` keeps U+0020, its oracle's verbatim regex). Remaining NOTES-L1 residuals, measured + test-pinned as deliberate divergences: bare NUMERIC refs (DOM decodes `&#39 x` → `' x`; CLI semicolon-required), UPPERCASE legacy forms (`&AMP;` — DOM decodes; CLI exact-lowercase), the ~2200 other named refs (`&copy;` …), the `&#38;amp` two-pass artifact (CLI `&`, DOM `&amp`; fix is a single scanner), and legacy-name-followed-by-alphanumeric (`&amplt` — DOM longest-matches to `&lt`; CLI keeps whole, deliberately: naive per-name decode without that guard would corrupt longer entities like `&notin;`/`&ltimes;`). Surrogate rejection is NOT a delta — `Unicode.Scalar` already nils. **Porting note (earned the hard way, two review rounds):** `NSRegularExpression` is NOT a drop-in for a JS regex. ICU applies FULL case folding under `.caseInsensitive` (U+017F folds to `s`, U+212A to `k`) where JS `/i` without `u` folds ASCII only; ICU matches `$` before a final line terminator regardless of `.anchorsMatchLines` where JS `$` without `m` matches at absolute end (`\z` is the correct spelling); ICU `\s` includes U+0085 and excludes U+FEFF where JS's does the opposite; and ICU `\b` derives from a Unicode word class where JS's is ASCII. Each of these must be swept across EVERY site in a ported function, not fixed only where first noticed — that omission is what round 2 caught. Where the semantics cannot be expressed at all (JS's backreference is ASCII-case-insensitive; ICU's cannot be), the port uses a scanner. |
...
| 13 | search-notes | C | `query`, `searchContent`(bool), `account`, `folder`, `modifiedSince`, `limit` | Title (or body) search; returns `{id,title,content:"",tags:[],created,modified,folder,account}[]` — apple-cli now emits **all eight keys verbatim**, including the oracle's hardcoded `content:""`/`tags:[]` (operator ruling D6/NOTES-M1, 2026-08-19; no longer a deviation). Flags `Recently Deleted`. Sync-aware. |
| 14 | get-selected-notes | C | — | Reads Notes.app **UI selection** → `{id,title,content:"",tags:[],created,modified,shared,passwordProtected,folder,account}[]` — **ten** keys (`build/index.js:40717-40728`); apple-cli emits **all ten verbatim**, including the oracle's hardcoded `content:""`/`tags:[]` (operator ruling D6/NOTES-M1, 2026-08-19; no longer a deviation). `folder`/`account` fall back to `undefined` on the oracle when empty and are `String?` here, so both sides omit the key rather than emitting null. |
...
- **search / selected / shared** — **RESOLVED 2026-08-19 (operator ruling D6/NOTES-M1): all three now emit the MCP's placeholder `content:""` and `tags:[]` verbatim, so nothing is dropped anywhere.** The rationale recorded below ("propagating empty placeholder fields is worse than omitting them") was the CLI's own judgement and has been **overruled in favour of strict parity** — a consumer ported from the MCP reads the oracle's full key set and must find that same set (**eight** keys on search, **ten** on selected, **nine** on shared — the counts differ per endpoint), and a placeholder the oracle itself always emits is part of the wire contract, not information. Cost of the fix was **zero**: the oracle hardcodes both fields (`build/index.js` search `:39813-39815`, shared `:40371-40381`, and the same for selected) and never fetches them, so the "per-hit AppleScript round-trip" this deviation was originally costed against does not exist on either side. The original rationale is preserved verbatim below as the audit record of why the code looked the way it did. **search** emits the note's real `created`/`modified` — the MCP reads both per-hit in its search loop and `new Date()` is only its unreadable-date fallback, so these are REAL fields the superset rule requires. (An earlier revision of this line claimed the MCP fabricated them at response time and dropped them on that basis; a review pass disproved the claim against the oracle source — `build/index.js` search loop emits `createdParts`/`modifiedParts` per row via `asDatePartsExpr` — and the fields were ported.) The **real** `account` value the MCP returns on search IS emitted (`NoteSummary.account`). **↓ RETRACTED RATIONALE — superseded by operator ruling D6/NOTES-M1 on 2026-08-19. Kept verbatim as the audit record of why the code once looked the way it did; it is NOT current guidance and must not be cited as one:** *"Rationale: propagating empty placeholder fields is worse than omitting them; every field apple-cli emits carries real information."* The premise that a placeholder carries no information is what was overruled — a field the oracle emits unconditionally is part of the wire contract, so parity, not editorial judgement about the field's usefulness, decides whether it ships.
