# apple mail rules update

Update a rule by index (patch; EXECUTES by default; --dry-run previews).

## Synopsis

```
apple mail rules update <index> [flags]
```

## Options

- `<index>`
  <br>1-based rule index (from `rules list`).
- `--action` `<action>` *(repeatable)*
  <br>Replacement action (repeatable; replaces all).
- `--condition` `<condition>` *(repeatable)*
  <br>Replacement condition (repeatable; replaces all).
- `--enabled`
  <br>Enable/disable the rule.
- `--match` `<match>`
  <br>Match logic: all or any.
- `--name` `<name>`
- `--no-enabled`
  <br>Enable/disable the rule.
