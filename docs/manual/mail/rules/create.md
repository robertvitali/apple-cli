# apple mail rules create

Create a rule (EXECUTES by default; --dry-run previews).

## Synopsis

```
apple mail rules create [flags]
```

## Options

- `--action` `<action>` *(repeatable)*
  <br>Action 'key=value' (repeatable): move_to/copy_to/mark_read/mark_flagged/flag_color/delete/forward_to.
- `--condition` `<condition>` *(repeatable)*
  <br>Condition 'field:operator:value[:header]' (repeatable).
- `--disabled`
  <br>Create the rule disabled.
- `--match` `<match>`
  <br>Match logic: all (AND) or any (OR).
- `--name` `<name>`
