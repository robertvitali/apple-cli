...
Manage drafts: list | create | send | open | delete (EXECUTES by default; --dry-run previews).

## Synopsis

```
apple mail draft <action> [flags]
```

## Options

- `<action>`
  <br>Action: list | create | send | open | delete.
- `--account` `<account>`
- `--bcc` `<bcc>` *(repeatable)*
- `--body` `<body>`
- `--cc` `<cc>` *(repeatable)*
- `--draft-subject` `<draft-subject>`
  <br>EXACT (case-insensitive) subject of the draft to send/open/delete — not a keyword/substring.
- `--subject` `<subject>`
- `--to` `<to>` *(repeatable)*
