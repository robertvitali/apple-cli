# apple mail analytics awaiting-reply

Sent messages with no reply yet (Sent↔Inbox cross-ref).

## Synopsis

```
apple mail analytics awaiting-reply [flags]
```
...
- `--days` `<days>`
  <br>Look back this many days over Sent.
- `--exclude-noreply`
  <br>Skip messages sent to noreply addresses (default on).
- `--max` `<max>`
  <br>Max results.
- `--no-exclude-noreply`
  <br>Skip messages sent to noreply addresses (default on).
