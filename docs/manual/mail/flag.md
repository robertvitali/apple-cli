...
Flag/unflag messages by id or --match, with optional color (EXECUTES by default; --dry-run previews).

## Synopsis

```
apple mail flag [<ids>...] [flags]
```
...
- `--color` `<color>`
  <br>Flag color: none/orange/red/yellow/blue/green/purple/gray.
- `--mailbox` `<mailbox>`
