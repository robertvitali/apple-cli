# apple mail templates save

Create or overwrite a template (EXECUTES by default; --dry-run previews).

## Synopsis

```
apple mail templates save <name> [flags]
```
...
- `<name>`
- `--body` `<body>`
  <br>Template body (may contain {placeholder} tokens).
- `--subject` `<subject>`
  <br>Optional subject template.
