# apple mail mailboxes create

Create a mailbox/folder (EXECUTES by default; --dry-run previews; nested via '/').

## Synopsis

```
apple mail mailboxes create [flags]
```
...
- `--account` `<account>`
- `--name` `<name>`
  <br>Mailbox name (may contain '/' for a nested path).
- `--parent` `<parent>`
  <br>Optional parent mailbox for nesting.
