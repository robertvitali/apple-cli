...
```
apple notes get-link [flags]
```

## Options

- `--account` `<account>`
  <br>Account containing the note (ignored if id is provided).
- `--id` `<id>`
  <br>Note id (preferred - more reliable than title).
- `--title` `<title>`
  <br>Note title (use id instead when available).
