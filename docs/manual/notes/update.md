...
REPLACE a note's body (and optional title), by --id or --title (EXECUTES; --dry-run previews).

## Synopsis

```
apple notes update [flags]
```

## Options

- `--account` `<account>`
  <br>Account (title path only).
- `--format` `<format>`
  <br>Content format: plaintext|html.
- `--id` `<id>`
  <br>Note id (preferred).
- `--new-content` `<new-content>`
  <br>New body (REPLACES the whole body).
- `--new-title` `<new-title>`
  <br>New title (plaintext format only).
- `--title` `<title>`
  <br>Current note title.
