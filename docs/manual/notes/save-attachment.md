# apple notes save-attachment

Write one attachment to disk, path must be under home/temp/Volumes (EXECUTES; --dry-run previews).

## Synopsis

```
apple notes save-attachment [flags]
```

## Options

- `--attachment-id` `<attachment-id>`
  <br>Attachment id (from `attachments`).
- `--note-id` `<note-id>`
  <br>CoreData note id.
- `--path` `<path>`
  <br>Absolute destination path.
