...
Create a note, title prepended as <h1> (EXECUTES; --dry-run previews).

## Synopsis

```
apple notes create <title> [flags]
```
...
- `<title>`
  <br>Note title.
- `--account` `<account>`
  <br>Account (defaults to iCloud).
- `--content` `<content>`
  <br>Note body.
- `--folder` `<folder>`
  <br>Folder (nested paths ok).
- `--format` `<format>`
  <br>Content format: plaintext|html.
- `--tags` `<tags>` *(repeatable)*
  <br>Echo-only tags (NOT persisted by Notes).
