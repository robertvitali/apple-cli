...
- `--content` `<content>`
  <br>Content to append.
- `--format` `<format>`
  <br>Content format: plaintext|html.
- `--id` `<id>`
  <br>Note id (preferred).
- `--position` `<position>`
  <br>Where to insert: after (default) appends, before prepends.
- `--separator` `<separator>`
  <br>String placed between existing and new content (default: two newlines).
- `--title` `<title>`
  <br>Note title.
