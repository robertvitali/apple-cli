...
| Q5d | **Contacts `cleanName` keeps combining marks the oracle deletes — and the obvious "consistency" fix makes it WORSE.** Found by review during Q5c. `cleanName` uses raw `CharacterSet.alphanumerics` (L*+M*+N*), so U+0301 survives; the oracle's `re.sub(r'[^\w\s\'-]','')` deletes it, because Python `\w` does not match a combining mark. Measured against live `fuzzy_match`: `jose`/`José`(NFD) oracle **1.0000** vs ours **0.7500**; `zoe`/`Zoë` oracle **1.0000** vs **0.6667**; `renee`/`Renée` oracle **1.0000** vs **0.8000**. **DO NOT "fix" this by pushing `[Unicode.Scalar]` through `sequenceRatio` for symmetry with Q5c** — measured, that moves two of the three FURTHER from the oracle (jose 0.75 -> 0.68, zoe 0.667 -> 0.6375). Grapheme counting is currently MASKING the character-class defect. The real fix is `cleanName` using the corrected `alnum` set (L*+N*) that `Fuzzy` already defines for `fullProcess`. More reachable than Q5c: NFD accented names arrive via CardDAV/vCard. Also still grapheme-based on that path: `sequenceRatio` `Array(a)`, and `matchContacts` `query.count`/`token.count` inside the 0.85/0.80 prefix formulas, plus `hasPrefix` (grapheme-canonical) vs Python `startswith` (code-point). | contacts | TODO | — |
