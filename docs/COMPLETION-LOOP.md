...
| Q4d | **SQLiteReader leaves PII snapshots on disk.** `copyToTemp` writes a full copy of the Envelope Index / `chat.db` to `$TMPDIR` and relies on `deinit` to remove it, but `deinit` does not run when ArgumentParser exits via `exit()`. Measured: **>150 files totalling multiple GB**, mode 0644 — real subjects/senders/recipients at rest. Not a cross-user leak (`$TMPDIR` is 0700) and macOS reaps `/var/folders` eventually, but copies outliving the process is the wrong default. Fix: unlink the temp file immediately after `sqlite3_open_v2` succeeds (the fd stays valid; the copy becomes unreachable and is reclaimed at exit) — no `atexit` handler needed. Touches the reader shared by all six domains, so it gets its own commit + review. | all | TODO | — |
