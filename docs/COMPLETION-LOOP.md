...
| Q4k | **The `.eml` temp directory — and the premise this row was filed on is HALF WRONG, which is the finding.** Filed after Q4i on the reasoning that `OwnedTempDir("apple-cli-eml")` holds COMPLETE RFC-822 messages on a 24h age reap and so has "the same hole Q4i just closed". **Ground truth says the two file classes differ and only one is a hole.** (1) The `--out`-less `.eml` temps are a DELIBERATE HAND-OFF: `openEml` runs `open -a Mail`, which returns immediately and leaves Mail to read the file *after this process is gone* — the surface even tells the operator "The .eml is kept at eml_path". Signal-unlinking those would destroy a live hand-off and break the compose window; the age reaper is the RIGHT mechanism there precisely because the owner is Mail.app, which cannot be `flock`ed or interrogated the way Q4i's own processes can. **So the Q4i treatment must NOT be applied to them.** (2) The `--gui-send` HTML temp IS a genuine target: it is `defer`-deleted because `sendHtmlViaGui` is synchronous, so nothing consumes it after we exit, and a signal death strands the operator's message body until the 24h sweep. The existing comment already anticipated exactly this ("a SIGKILL or a panic leaves it behind") and accepted the reaper as the mitigation; Q4i's registry makes immediate removal available instead. **Remaining work, now correctly scoped:** separate "paths to unlink" from "the one directory to remove" in `SignalSafeCleanup` (its containment guard is currently hard-scoped to the snapshot session directory, so an eml-dir path is refused, and the shared eml directory must NEVER be `rmdir`d — the trap review flagged), register only the gui-send HTML temps, and pin in the logic tier that the hand-off `.eml` paths are NOT registered — that negative is the safety-critical half. Live validation of `--gui-send` itself stays operator-present (D3). | mail | TODO | — |
