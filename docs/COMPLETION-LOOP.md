...
| Q5c | **`Fuzzy` counts GRAPHEME CLUSTERS where the oracle counts CODE POINTS.** Found by review during Q5b, deferred because the fix is a type change across the whole WRatio path, not a one-liner, and it deserves its own iteration. `partialRatio`/`ratio` do `Array(String)` and `wRatio` does `p1.count` — all Characters. Python `len()` is code points. Repro with no combining-mark involvement: `"\u{1100}\u{1161}\u{1100}\u{1161}"` (Hangul jamo) — both sides keep all four scalars, but Python len=4 and Swift count=2, which changes `lenRatio` AND the `ratio` denominator. Reviewer measured 13/15 divergent cases on an earlier build; the M* subset of those is FIXED (Q5b now uses L*+N* via `alnum`), this row is the residue that survives that fix. **Not currently reachable from the corpus** — 0 of 10,115 bodies retain a multi-scalar grapheme after `cleanText` — so it is latent, and it fires on NFD text (macOS paste), Hangul, and emoji ZWJ sequences. **Fix:** carry `[Unicode.Scalar]` through `fullProcess` -> `ratio`/`partialRatio`/`wRatio` instead of `[Character]`. Needs its own red-proof and a live-oracle re-diff, since it moves every length in the scorer. | messages | TODO | — |
