...
| Q4k | **The `.eml` temp directory has the same hole Q4i just closed, and holds strictly more.** `OwnedTempDir("apple-cli-eml")` holds COMPLETE RFC-822 messages — headers and bodies, not a 500-char preview — and is reaped only by a 24-hour age sweep; the `--gui-send` HTML temps are `defer`-deleted, which a signal death misses for exactly the reason `atexit` does. So a Ctrl-C during `mail compose`/`--gui-send` leaves full message bodies on disk for up to a day. Q4i's literal scope was the snapshot directory, so this is out of its scope — but Q4i's RATIONALE ("it holds copies of the operator's mail at 0600") applies verbatim and more strongly. Raised by review; filed rather than absorbed. Likely shape: register those paths with the same `SignalSafeCleanup` registry rather than a second bespoke mechanism — which is also the question of whether that type belongs to `SQLiteReader` at all. **DO NOT do that literally without first splitting the registry's two jobs.** Review flagged this as the single most likely next defect in this code: the registry holds exactly ONE `dir` and ONE `lockFile`, and the handler's tail is snapshot-specific (unlink `.lock`, `rmdir` the directory, restore `.lock` if `rmdir` fails). `apple-cli-eml` is SHARED and long-lived, has no `.lock`, and must never be `rmdir`d — so reusing the registry as-is either removes a shared directory or bolts a second special case onto the handler tail. Separate "paths to unlink" from "the one directory to remove" FIRST, while the type still has a single caller. | mail | TODO | — |
