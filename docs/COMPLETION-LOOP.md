...
| Q4d | **SQLiteReader leaves PII snapshots in `$TMPDIR`.** `copyToTemp` copies the Envelope Index / `chat.db` / NoteStore and relies on `deinit`, which never runs because ArgumentParser exits via `exit()`. Measured: >150 snapshot files totalling multiple GB, mode 0644 — real subjects/senders/recipients. **ATTEMPTED AND REVERTED once; read this before retrying.** Three designs were tested against the real code, not reasoned about: (a) *unlink right after `sqlite3_open_v2`* — FAILS, `disk I/O error`: the plain read-only open needs the copied `-wal`, which SQLite opens lazily. (b) *atexit registry + hourly sweep* — builds and passes, but UNSOUND: `FileManager.copyItem` PRESERVES the source mtime (verified: 3h-old source → 3h-old copy), so a snapshot of a store not written recently (`NoteStore.sqlite` was 20h stale) is born past the threshold and a second concurrent `apple` process deletes a live reader's files. Fixable by restamping mtime after copy. (c) *checkpoint the copy → `immutable=1` → unlink immediately* — the elegant one, and it FAILS in C: `PRAGMA wal_checkpoint(TRUNCATE)` works and a warm read after open works, but SQLite still reads pages lazily as later queries touch them, so `sqlite3_step` fails after the unlink. A Python probe wrongly suggested otherwise because `sqlite3.connect()` reads eagerly where `sqlite3_open_v2` does not. **Recommended next attempt:** design (b) WITH `setAttributes([.modificationDate: Date(), .posixPermissions: 0o600])` after each copy, snapshots relocated to an owned `$TMPDIR/apple-cli-snapshots/` created 0700 (which also removes the shared-namespace clash with `apple-cli-fixture-*` test files and the 148k-entry scan cost), a `.typeRegular` guard before `removeItem`, and the sweep made internal rather than public. Known remaining gaps to document rather than fix: `atexit` does not run on SIGINT/SIGTERM, and retention is unbounded if no further snapshot-backed command runs. | all | TODO | — |
