...
| Q4l | **Residual Q4k review findings, none blocking, filed rather than absorbed.** From the three-reviewer pass on Q4k. **MEDIUM:** the containment boundary widened from a directory this process owns exclusively to one SHARED with concurrent `apple` invocations, so the guard proves less than it did and the comment does not say so; and the two most novel paths (file-only arming, late directory adoption) have no deterministic test. **LOW:** `track`'s `hasPrefix` guard does not normalise `..`, and this change routes env-controlled text (`TestMode.sandboxPrefix`) into a tracked filename — security rated it LOW because the components are UUIDs and the prefix is operator-set, but the guard is `..`-blind; `roots` grows monotonically and retains never-created and deleted directories; `registerRoot` couples "containment boundary" with "arm the process", so a path-computation helper now installs four signal dispositions even when it has nothing to protect; `disarmForRetry`'s doc still overclaims. **From the critic, worth its own look:** the corrected premise may itself be over-general — `send --gui-send` also writes a `.eml` that nothing ever reads, so the hand-off exclusion may be broader than it needs to be; and the safety-critical negative is pinned at the URL factory rather than at the site where the mistake would actually be made. | mail | TODO | — |
