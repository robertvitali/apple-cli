...
| Q4e | **`hasQuestion` is dead for message bodies.** `analyticsRows` never SELECTs a `snippet` column (no `summaries` join), yet `analyticsRow` reads `row["snippet"]` — so it is nil for every row and `Analytics.hasQuestion` collapses to a subject-only `?` test. The oracle scans the first 500 chars of message content (`smart_inbox.py:335-341`), so `"MEDIUM (contains question)"` can never fire on a body-only question and `priorityScore` permanently loses its 2-point body term. A dropped oracle capability. The doc comments at `Analytics.swift` (Row.snippet, hasQuestion) currently assert the opposite — corrected in the Q4b commit so the code stops lying while the gap stays open. Fix: join `summaries` in `analyticsRows`, or record the divergence in the port spec. | mail | TODO | — |
| Q4f | **`emlDestURL` leaks full RFC-822 messages.** Found during the Q4d review: 244 `.eml` files written with no cleanup by design and no sweep coverage. Unlike the SQLite snapshots these are complete message bodies. Confirm the count, decide whether the write is intentional operator-facing output (in which case document the location) or an internal temp that should be cleaned. | mail | TODO | — |
| Q4g | **The legacy flat-name leak is only cleaned on THIS machine.** The new reaper looks exclusively inside `$TMPDIR/apple-cli-snapshots/`; the original 166 leaked files sat in the `$TMPDIR` ROOT as `apple-cli-<uuid>.sqlite`. Any other machine that ran a pre-fix build keeps that mail content indefinitely, and nothing will ever collect it. Decide between a one-shot legacy sweep behind a marker file (note: enumerating a 148k-entry `$TMPDIR` measured 0.24s, so once per machine, not per run) and an explicit `apple doctor --clean-temp`. If the tool has only ever run on this machine, close the item saying so rather than inheriting it. | all | TODO | — |
| Q4h | **Snapshot coherence is unverified.** The main DB and its `-wal`/`-shm` are copied at three different instants, so Mail can commit — or checkpoint — between them, yielding a pre-checkpoint main file beside a post-checkpoint WAL of a different generation. SQLite documents file-level copies of a live DB as capable of producing inconsistent or corrupt results; outcomes range from a silently stale read to `SQLITE_CORRUPT`. Pre-existing, not introduced by Q4d. `VACUUM INTO` / the backup API would produce ONE consistent file with no sidecars at all — deleting the sidecar copy, its cleanup and this whole class — at the cost of a page-by-page read (seconds for 321 MB) versus the current APFS clonefile (0.2 ms for 208 MB). Measure and decide explicitly. | all | TODO | — |
| Q4i | **`atexit` does not run on SIGINT/SIGTERM.** Ctrl-C during a slow `apple mail search` over a large index is the most likely abnormal exit this tool will see, and it strands a session directory until the next run reaps it. ~6 lines: a handler that calls `SnapshotSession.remove()` then re-raises with the default disposition, using async-signal-safe `unlink(2)` on pre-rendered C strings rather than `FileManager`. SIGKILL stays uncatchable — that is what the reaper is for. | all | TODO | — |
